...
public class CommandFloorExecutor implements CommandExecutor {

    private WorldEditPlugin we;

    public CommandFloorExecutor(WorldEditPlus plugin) {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0 || (args.length == 1 && args[0].equalsIgnoreCase("-w"))) {
            player.sendMessage(ChatColor.RED + "To few arguments. Use: //floor [-w] <material>");
            return true;
        }
        ...
        player.sendMessage(ChatColor.LIGHT_PURPLE + "Floor created.");
